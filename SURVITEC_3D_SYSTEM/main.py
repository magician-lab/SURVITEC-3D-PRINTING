...
#     app_folder = os.path.join(base_path, "SchoolSystem")
...
def edit_school(school_id):
    ...
    if duplicate:
        ...
        flash(
            "Another school already uses this school code.",
            "danger"
        )
...
def manage_school_admins():
    ...
    if request.method == "POST":
        ...
        email_exists=SchoolAdmin.query.filter_by(
            email=email
        ).first()
...
def add_student(school_id):
    ...
    if date_of_birth:
        ...
        date_of_birth = kenya_tz.localize(
    ...
    token = generate_student_consent(student)
    ...
    if token:
        ...
        print("Consent token was not created.")
...
@app.route("/material_schools/<material_id>")
@system_admin_required
...
@app.route("/download_material/<id>")
@system_admin_required
...
def student_login():
    ...
    if request.method == "POST":
        ...
        ########################################################
        # HAS THIS CONSENT BEEN ACCEPTED?
        ########################################################

        agreed = ConsentLog.query.filter_by(
        ...
        token = ConsentToken.query.filter_by(
        ...
        if target == "Parent":
            ...
            flash(
                ...
                "Parental consent is required before access is granted.",
        ...
        flash(
            ...
            "Please review and accept the consent before continuing.",
...
@app.route("/student/material/content/<id>")
@student_login_required
...
@app.route("/student/material/download/<id>")
@student_login_required
...
@app.route("/survey/<id>/toggle", methods=["POST"])
@system_admin_required
...
@app.route("/survey/<id>/delete", methods=["POST"])
@system_admin_required
...
@app.route("/survey/<survey_id>/question/add", methods=["POST"])
@system_admin_required
...
@app.route("/student/survey/<id>/start")
@student_login_required
def start_survey(id):
...
@app.route("/student/survey/attempt/<attempt_id>")
@student_login_required
...
@app.route(
    "/student/survey/<attempt_id>/submit",
    methods=["POST"]
)
@student_login_required
...
@app.route("/student/result/<survey_id>")
@student_login_required
...
@app.route("/student/survey/<survey_id>/retake")
@student_login_required
...
def school_login():
    ...
    if request.method == "POST":
        ...
        consent = Consent.query.filter_by(
            ...
            Consent.version.desc()
        ).first()

        ####################################################
        # IF NO ACTIVE CONSENT
        ####################################################
        ...
        if consent is None:
            ...
            session["school_admin_name"] = admin.username
            session["school_id"] = school.id
            session["school_name"] = school.name
        ...
        existing = ConsentLog.query.filter_by(
            consent_id=consent.id,
            school_id=school.id,
            user_id=admin.id,
        ...
        session["pending_school_admin"] = admin.id
        session["pending_school_id"] = school.id
        session["pending_consent"] = consent.id

        flash(
            "Please accept the latest consent before continuing.",
            "warning"
        )
...
def view_consent(token):

    consent_token = ConsentToken.query.filter_by(
        token=token
    ).first()
    ...
    if now > kenya_datetime(consent_token.expires_at):

        consent_token.status = "Expired"
    ...
    if consent_token.status == "Accepted":
    ...
    if consent_token.status == "Declined":
    ...
    if consent_token.opened_at is None:

        consent_token.opened_at = now

        consent_token.status = "Opened"

        consent_token.ip_address = request.headers.get(
            "X-Forwarded-For",
            request.remote_addr
        )

        consent_token.user_agent = request.headers.get(
            "User-Agent"
        )

        consent_token.accepted_language = request.headers.get(
            "Accept-Language"
        )

        ua = request.user_agent

        consent_token.browser = ua.browser

        consent_token.operating_system = ua.platform

        consent_token.device = ua.string
    ...
    if consent_token.user_type == "Student":

        user = Student.query.get(
            consent_token.user_id
        )

    elif consent_token.user_type == "SchoolAdmin":

        user = SchoolAdmin.query.get(
            consent_token.user_id
        )
    ...
    if request.method == "POST":
        ...
        if action == "agree":
            ...
            consent_token.status = "Accepted"

            consent_token.accepted_at = now

            user.consent_completed = True

            user.consent_completed_at = now

            user.last_consent_id = consent.id
            ...
            log = ConsentLog(
                ...
                user_type=consent_token.user_type,

                agreed=True,

                consented_at=now,

                ip_address=consent_token.ip_address,

                browser=consent_token.browser,

                operating_system=consent_token.operating_system,

                device=consent_token.device,

                user_agent=consent_token.user_agent,

                accepted_language=consent_token.accepted_language
            ...
            consent_token.status = "Declined"

            consent_token.declined_at = now
...
@app.route("/consent/accepted/<token>")
def consent_accepted(token):
...
@app.route("/consent/declined/<token>")
def consent_declined(token):
...
@app.route("/consent/access-denied/<token>")
def consent_access_denied(token):
...
@app.route("/consent/parent/<token>")
def parent_required(token):
    ...
    return render_template(
        ...
        "parent_required.html",

        token=consent_token

    )

@app.route("/consent/send-parent/<token>")
def send_parent_consent(token):
    ...
    flash(
        ...
        "Consent email has been sent to the parent successfully.",
...
@app.route("/consent/pending/<token>")
def consent_pending(token):
    ...
    return render_template(
        ...
        email=consent_token.recipient_email

    )

@app.route("/system/consents")
@system_admin_required
def manage_consents():
...
def create_consent():
    ...
    if effective_date:
        ...
        effective_date = kenya_tz.localize(
...
def edit_consent(id):
    ...
    if request.method == "POST":
        ...
        consent.title = request.form["title"]

        consent.target = request.form["target"]

        consent.version = request.form["version"]

        consent.content = request.form["content"]

        consent.effective_date = request.form["effective_date"]
...
@app.route("/system/consent/<id>/activate")
@system_admin_required
def activate_consent(id):
...
@app.route("/system/consent/<id>/deactivate")
@system_admin_required
def deactivate_consent(id):
...
@app.route("/system/consent/<id>/delete")
@system_admin_required
def delete_consent(id):

    consent=Consent.query.get_or_404(id)

    used=ConsentLog.query.filter_by(
    ...
    if used>0:
        ...
        flash(
            ...
            "Cannot delete a consent that has already been used.",
...
@app.route("/system/consent/<id>/duplicate")
@system_admin_required
def duplicate_consent(id):
    ...
    new=Consent(
        ...
        version=f"{consent.version} Copy",
    ...
    flash(
        ...
        "Consent duplicated successfully.",
    ...
    return redirect(
        ...
        url_for("edit_consent",id=new.id)

    )

@app.route("/system/consent/<consent_id>")
@system_admin_required
def admin_consent_details(consent_id):

    consent = Consent.query.get_or_404(consent_id)

    accepted = ConsentLog.query.filter_by(
        consent_id=consent_id,
        agreed=True
    ).all()

    declined = ConsentLog.query.filter_by(
        consent_id=consent_id,
        agreed=False
    ).all()

    pending = ConsentToken.query.filter_by(
        consent_id=consent_id
    ).filter(
        ConsentToken.status.in_(
            [
                "Created",
                "Sent",
                "Opened"
    ...
    return render_template(
        "consent_details.html",
        consent=consent,
        accepted_logs=accepted,
        declined_logs=declined,
...
@app.route("/school-admin/consent/<token>")
def school_admin_consent(token):
...
@app.route("/consent/request/<string:user_type>/user_id>")
def request_new_consent(user_type, user_id):
    ...
    if user_type == "Student":
        ...
        user = SchoolAdmin.query.get_or_404(user_id)
...
def consent_logs():
    ...
    logs=ConsentLog.query.order_by(

        ConsentLog.consented_at.desc()
...
@app.route("/parent/consent/sent")
def parent_consent_sent():

    return render_template(
        "parent_consent_sent.html"
    )

# from sqlalchemy import text

# @app.route("/system/rebuild_students_table")
...
#         db.session.execute(text("DROP TABLE IF EXISTS survey_answers"))
#         db.session.execute(text("DROP TABLE IF EXISTS survey_attempts"))
#         db.session.execute(text("DROP TABLE IF EXISTS consent_logs"))
...
#         ###################################################
#         # RECREATE FROM SQLALCHEMY MODEL
#         ###################################################
...
#         return "✅ Students table rebuilt successfully."
